...
class KiwiWaterfallRecorder(KiwiSDRStream):
    ...
    def _setup_rx_params(self):
        ...
        self._freq_bins = self._freq + (
            0.5 + np.arange(self.WF_BINS)
        ) / self.WF_BINS * self.zoom_to_span(self._options.zoom)
    ...
    def _process_waterfall_samples(self, seq, samples):
        ...
        self._queue.put(
            {"seq": seq, "freq_bins": self._freq_bins, "wf_samples": samples}
        )
...
class Consumer(threading.Thread):
    ...
    def proc(self):
        ...
        if self._wf_data["seq"] in self._store:
            ...
            logging.info(
                "found seq %d %f %d (%d|%d,%d)"
                % (
                    self._wf_data["seq"],
                    ts,
                    len(self._wf_data["wf_samples"]),
                    len(self._store),
                    self._wf_queue.qsize(),
                    self._snd_queue.qsize(),
                )
            )
    ...
    def _get_output_filename(self):
        ...
        return "%s_%d-%d%s.npy" % (
            time.strftime("%Y%m%dT%H%M%SZ", self._start_ts),
...
def main():
    ...
    parser.add_option(
        "-z", "--zoom", dest="zoom", type="int", default=0, help="Zoom level 0-14"
    )
    parser.add_option(
        "--station",
        dest="station",
    ...
    logging.basicConfig(
        level=logging.getLevelName(opt.log_level.upper()), format=FORMAT
    )
    ...
    wf_recorder = KiwiWorker(
        args=(KiwiWaterfallRecorder(opt, wf_queue), opt, run_event)
    )
