...
indexAdjustTable = [
    ...
    -1,
    -1,
    -1,  # +0 - +3, decrease the step size
    2,
    4,
    6,
    8,  # +4 - +7, increase the step size
    -1,
    -1,
    -1,
    -1,  # -0 - -3, decrease the step size
    ...
    def _set_auth(self, client_type, password="", tlimit_password=""):
        if tlimit_password != "":
            if password == "":
                ...
                password = "#"
            self._send_message(
                "SET auth t=%s p=%s ipl=%s" % (client_type, password, tlimit_password)
            )
        else:
...
class KiwiSDRStream(KiwiSDRStreamBase):
    ...
    def set_agc(self, on=False, hang=False, thresh=-100, slope=6, decay=1000, gain=50):
        logging.debug(
            "set_agc: on=%s hang=%s thresh=%d slope=%d decay=%d gain=%d"
    ...
    def start_frequency_to_counter(self, start_frequency):
        ...
        counter = round(
            start_frequency / self.MAX_FREQ * 2 ** self.MAX_ZOOM * self.WF_BINS
        )
    ...
    def _process_msg_param(self, name, value):
        if name == "load_cfg":
            ...
            self._gps_pos = [
                float(x) for x in urllib.unquote(d["rx_gps"])[1:-1].split(",")[0:2]
            ]
            if self._options.idx == 0:
                logging.info(
                    "GNSS position: lat,lon=[%+6.2f, %+7.2f]"
        ...
        if name == "too_busy":
            raise KiwiTooBusyError(
                "%s: all %s client slots taken" % (self._options.server_host, value)
            )
        ...
        if name == "audio_rate":
            ...
            if (
                self._options.idx == 0
                and self._version_major is not None
                and self._version_minor is not None
            ):
                logging.info(
                    "Server version: %d.%d", self._version_major, self._version_minor
                )
    ...
    def _process_aud(self, body):
        (
            flags,
            seq,
        ) = struct.unpack("<BI", buffer(body[0:5]))
            ...
            if self._options.S_meter == 0 and self._options.sdt == 0:
                ...
                if self._options.S_meter >= 0:
                    ...
                    if self._options.sdt != 0:
                        interval = (self._start_sm_ts is not None) and (
                            sec_of_day(now) // self._options.sdt
                            != sec_of_day(self._start_sm_ts) // self._options.sdt
                        )
                    ...
                    if self._s_meter_avgs == self._options.S_meter or interval:
                        ts = (
                            time.strftime("%d-%b-%Y %H:%M:%S UTC ", now)
                            if self._options.tstamp
                            else ""
                        )
                        if self._options.stats and self._options.sdt:
                            print(
                                "%sRSSI: %6.1f %.1f meas/sec"
                                % (ts, self._s_meter_cma, meas_sec)
                            )
                        else:
        ...
        if self._modulation == "iq":
            gps = dict(
                zip(
                    ["last_gps_solution", "dummy", "gpssec", "gpsnsec"],
                    struct.unpack("<BBII", buffer(data[0:10])),
                )
            )
            data = data[10:]
            ...
            if self._options.raw is True:
                ...
                if self._compression:
                    ...
                    count = len(data) // 2
                    samples = np.ndarray(count, dtype=">h", buffer=data).astype(
                        np.int16
                    )
    ...
    def _process_wf(self, body):
        (
            ...
            flags_x_zoom_server,
            seq,
        ) = struct.unpack("<III", buffer(body[0:12]))
        data = body[12:]
    ...
    def _setup_rx_params(self):
        if self._type == "W/F":
            ...
            self._set_wf_interp(
                13
            )  # drop sampling + CIC compensation (Kiwi UI default)
        if self._type == "SND":
    ...
    def open(self):
        if self._type == "SND" or self._type == "W/F" or self._type == "EXT":
            ...
            self._set_auth(
                "kiwi", self._options.password, self._options.tlimit_password
            )
    ...
    def run(self):
        ...
        if self._reader:
            try:
                ...
                if received is None:
                    self._socket.close()
                    raise KiwiServerTerminatedConnection(
        ...
        time_limit = (
            ...
            and time.time() - self._start_time > tlimit
        )
        if time_limit or self._stop:
            try:
                if (
                    self._options.stats
                    and self._tot_meas_count > 0
                    and self._start_time != None
                ):
                    print(
                        "%.1f meas/sec"
                        % (
                            float(self._tot_meas_count)
                            / (time.time() - self._start_time)
                        )
                    )
            except AttributeError as e:  # for shutting down wfrecorder safely
                pass
